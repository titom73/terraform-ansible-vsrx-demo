...
resource "aws_eip" "vsrx1_public" {
  vpc     = true
  network_interface = "${aws_network_interface.vsrx1_fxp0.id}"
}

resource "aws_network_interface" "vsrx1_fxp0" {
  subnet_id = "${module.user_vpc.net_public_id}"
  private_ips = ["10.0.255.21"]
...
resource "aws_network_interface" "vsrx1_untrust" {
...
resource "aws_instance" "vsrx1" {
  depends_on = ["aws_instance.jdevops"]
  ...
  network_interface {
     device_index = 0
     network_interface_id = "${aws_network_interface.vsrx1_fxp0.id}"
  }
  network_interface {
    device_index = 1
    network_interface_id = "${aws_network_interface.vsrx1_untrust.id}"
  }
  tags {
    Name = "vsrx1-${var.stack}",
    stack = "${var.stack}",
    Os = "junos"
  }
  provisioner "local-exec" {
    command = "sleep 600 ; ansible-playbook ../../../../provisioning/jdevops.days/pb.vsrx.configure.yml -i ../../../../provisioning/jdevops.days/inventory --limit ${aws_instance.vsrx1.public_ip}"
  }
}
